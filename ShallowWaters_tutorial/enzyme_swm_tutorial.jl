...
# ╔═╡ 608b23db-2d25-4fea-a85d-198fc600b88c
using Parameters
...
md"""
The differentiable model we'll examine in this tutorial is a **shallow water model**. This means we're examining a simplified ocean model: imagine a box of fluid with wind-driven circulation. The equations of motion are given by 
"""
...
\text{Dissipation terms:}& \; \; \mathbf{M} = (M_x, M_y)
\end{align}
```
"""
...
md"""
Now we want to set up the foundation for computing derivatives. Let's begin by adding two key packages, Enzyme.jl and Checkpointing.jl:
"""

# ╔═╡ e531a2aa-2825-4360-b362-48845c69e353
md"""
Enzyme.jl is needed to generate and compile the model derivative code, Checkpointing.jl is needed to checkpoint the backwards run during the model differentiation. We need Checkpointing here because our model is nonlinear and storing the states for the reverse pass would cause us to run out of memory.

In addition to just adding Enzyme, there are a couple of Enzyme flags that we need to make the code run:
"""

# ╔═╡ f246c043-2c8a-412b-9f14-ddaf82ca07ea
begin
...
md"""
The first one has to do with type evaluation of variables during the differentiation process, and the second has to do with deducing which variables are required in the differentiation process.

Beginning with `looseTypeAnalysis!`, by setting this flag to `true`, we're telling Enzyme to make it's best guess whenever it comes across a variable that it's not sure the type of, rather than throwing an error. 

The second, `runtimeActivity!`, tells any load/stores to check during the derivative process (runtime) whether a variable is active (needed for differentiation) or constant (not needed for differentiation). Usually this process of checking the activity of a variable is done during the compile time, but for various reasons this is not sufficient enough for some code (including this one).

For more info on these the Enzyme FAQ has some handy information: https://enzyme.mit.edu/julia/stable/api/#Enzyme.API.looseTypeAnalysis!-Tuple{Any}.
"""

# ╔═╡ ef0d0b5d-c73e-4542-8e00-89eab440a836
md"""
## Modifiying the integration
"""

# ╔═╡ 1cc5083d-fdae-46f9-80f9-34efe83e82ea
md"""
One last step is needed before differentiating, we need a modified version of the integration defined in ShallowWaters.jl, one that

1. implements Checkpointing.jl during the integration, and

2. returns the value of a loss function `J`, rather than the prognostic variables

To this end, we have two functions:

1. `checkpointed_integration` initializes the integration process, and 

2. `loop` will be the checkpointed timestepping loop. The return value, or loss function, is defined at the end of `loop`. 

In summary, we're just taking the integration function from ShallowWaters.jl and modifying it in two places.
"""

# ╔═╡ 84d33d74-d315-4a26-bcfc-c11a10e862f8
...
# ╔═╡ 8cf228d2-b071-4382-bd5e-4722994a7d8a
...
# ╔═╡ e103bf60-36fe-4c96-a9c1-6f4cc3c28d73
md"""
...
md"""
Now we're ready to compute derivatives! I chose the loss 
```math
J = \frac{1}{nx \cdot ny}\sum_{x, y} u^2(t_f, x, y) + v^2(t_f, x, y).
```
This means we can compute gradients like 
```math
\frac{\partial J}{\partial u(t_0)}, \; \; \; \frac{\partial J}{\partial v(t_0)}
```
We'll compute these in two ways: 

1. from at-rest initial conditions, $u(t_0), \; v(t_0), \; \eta(t_0)$ all zeros,  and 

2. from a spinup state, i.e. we've integrated for ten years and begin from there. 
"""

# ╔═╡ 64be2a15-ddc7-4c85-b412-d84091910971
md"""
### Derivative 1: Initial condition sensitivity from rest
"""
...
derivs.u', 
c=:balance,
clim=(-abs(maximum(derivs.u)),abs(maximum(derivs.u))),
xlabel=("x (km)"),
ylabel=("y (km)"),
title=("∂J / ∂u(t_0)")
)
...
md"""
## Derivative 2: Initial condition sensitivity after spinup
"""

# ╔═╡ a3297c2c-d814-4d80-a559-c2ff27333fe8
md"""
Let's now compute a derivative beginning from a non-zero initial condition. To do so, we'll start from a ten year spinup.
"""
...
begin 
    ...
    Ndays = 30,
    initial_cond="ncfile",
    initpath="./initial_conditions/"
	)

	dS2 = Enzyme.make_zero(S)
	
	snaps2 = Int(floor(sqrt(S.grid.nt)))
	revolve2 = Revolve{ShallowWaters.ModelSetup}(S.grid.nt, snaps2; 
	    verbose=1, 
	    gc=true, 
	    write_checkpoints=false
	)
...
md"""
It's always a good idea to check the derivatives an automatic differentiation tool returns, so for brevity let's try and check them using finite differences. The idea here is that we use a first order Taylor series approximation to compute the same thing Enzyme just computed. Namely, if `J` is our loss then we know from the Taylor series that 
```math
J(\mathbf{x}(t_0) + \varepsilon) \approx J(\mathbf{x}(t_0)) + \varepsilon \frac{\partial J}{\partial \mathbf{x}(t_0)}
```
and rearranging this gives 
```math
\frac{\partial J}{\partial \mathbf{x}(t_0)} \approx \frac{J(\mathbf{x}(t_0) + \varepsilon) - J(\mathbf{x}(t_0))}{\varepsilon}
```
meaning for sufficiently small $\varepsilon$ we expect finite differences be roughly the same value as Enzyme. 
"""
...
begin
	...
	snaps3 = Int(floor(sqrt(S_outer.grid.nt)))
	revolve3 = Revolve{ShallowWaters.ModelSetup}(S_outer.grid.nt, snaps3; 
	    verbose=1, 
	    gc=true, 
	    write_checkpoints=false
	)
	...
	for s in steps
	    ...
	    J_inner = checkpointed_integration(S_inner, revolve3)
...
git-tree-sha1 = "0ed40abf91d84e02ee2f11eeb6552a823548d49f"
...
git-tree-sha1 = "366b02be78e08daf2e8d86b680568b6a14990ff7"
...
git-tree-sha1 = "aad065945dbddd8a48707468fae31fa5055e6cce"
uuid = "56019723-2d87-4a65-81ff-59d5d8913e3c"
version = "0.5.3"
weakdeps = ["Checkpointing"]
